 ...
 *
 * Autenticato con la sessione dell'utente; la scrittura la fa il server
 * con la chiave di servizio (le transizioni non passano MAI dal client),
 * ma solo dopo aver controllato che la pratica sia davvero sua.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!utente) {
      return NextResponse.json({ errore: "Devi essere collegato." }, { status: 401 });
    }
    if (!SERVIZIO_ATTIVO) {
      return NextResponse.json({ errore: "Il server non è configurato." }, { status: 503 });
    }
    ...
    if (typeof id !== "string" || !id) {
      return NextResponse.json({ errore: "Manca pratica_id." }, { status: 400 });
    }
    ...
    if (GIA_OLTRE.includes(pratica.stato)) {
      return NextResponse.json({ ok: true, stato: pratica.stato, nota: "Invio già registrato." });
    }
    if (pratica.stato === "creata") {
      return NextResponse.json({ errore: "La pratica non risulta pagata." }, { status: 409 });
    }
    ...
    if (!esito.ok) {
      ...
      return NextResponse.json({ errore: "Non sono riuscito a salvare. Riprova." }, { status: 503 });
    }

    return NextResponse.json({ ok: true, stato: "inviata" });
  } catch (e) {
    console.error("[pratiche] conferma invio, errore inatteso:", e);
    return NextResponse.json({ errore: "Errore inatteso. Riprova." }, { status: 500 });
  }
}
