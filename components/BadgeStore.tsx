...
function Mela() {
  return (
    <svg viewBox="0 0 24 24" className="size-[26px] shrink-0" aria-hidden="true">
      <path
        fill="currentColor"
...
function Badge({
  ...
  return (
    <span
      className="inline-flex select-none items-center gap-2.5 rounded-[11px] border border-white/14 bg-white/6 px-[18px] py-3 text-white"
      title="Non è ancora negli store: per ora si aggiunge alla schermata Home"
    >
      <Icona />
      <span className="flex flex-col leading-none">
        <span className="text-[10.5px] text-white/55">{sopra}</span>
        <span className="mt-0.5 text-[16px] font-medium tracking-[-0.01em]">{sotto}</span>
      </span>
    </span>
  );
}
