  ...
  return (
    <div className="w-full max-w-[420px]">
      ...
      {/* ⚠️ QUESTA NON È PIÙ UNA VIA SECONDARIA, ed è il motivo per cui
          ...
          Google non c'è più (scelta di Valerio: costava mezz'ora di
          lavoro su Google Cloud per una comodità che il link via email
          dà già). Quindi questa è LA strada per chi non vuole inventarsi
          l'ennesima password, e va vista. */}
      <div className="mt-7 border-t border-bordo pt-6">
        <button
          type="button"
          onClick={() => setModo(modo === "magico" ? "accedi" : "magico")}
          className="inline-flex w-full items-center justify-center gap-2.5 rounded-bottone border border-bordo bg-white px-4 py-3.5 text-[15px] font-medium text-inchiostro transition-all duration-200 hover:-translate-y-0.5 hover:border-verde/40 hover:shadow-[0_10px_24px_-14px_rgba(5,46,31,.35)] active:translate-y-0"
        >
          <Mail className="size-[18px] shrink-0 text-verde" aria-hidden="true" />
          {modo === "magico" ? "Torna alla password" : "Entra senza password"}
        </button>
        {modo !== "magico" && (
          <p className="mt-2.5 text-center text-[13px] text-fumo-2">
            Ti mandiamo un link via email: lo apri e sei dentro.
          </p>
        )}
      </div>
    </div>
