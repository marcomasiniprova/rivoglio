...
| L | 🔴 **Il ramo di lavoro non è mai stato unito a `main`** | `main` è fermo 79 commit indietro, al pivot del nome. Tutto (motore, blog, pagine evento, sicurezza) vive solo su `claude/saas-app-repo-analysis-ghehqa`. Il sintomo visibile: la scheda **Actions** di GitHub è vuota, perché i lavori programmati si leggono SOLO dal ramo predefinito. Serve una decisione di Valerio: unire il ramo a `main`. |
