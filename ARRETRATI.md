...
| A0 | 🔴 **Polar ha rifiutato il caso d'uso** | Il controllo automatico dell'iscrizione risponde "Use case not supported": categoria reclami di viaggio, a restrizione. Blocca l'incasso, cioè l'obiettivo cassa. Tre strade in `PAGAMENTI.md`, con l'email di richiesta di revisione già scritta. **Non si riscrive la descrizione per passare**: Polar è il venditore ufficiale e alla verifica guarda il sito vero. Decisione di Valerio. |
...
| P | **I nove enti nazionali che mancano** | Croazia, Slovenia, Slovacchia, Romania, Cipro, Estonia, Lettonia, Lituania, Liechtenstein. Da qui l'elenco ufficiale è bloccato dal proxy e **non si scrivono a memoria**: mandare una persona all'ufficio sbagliato è peggio che dirle di cercarlo. Apri `https://transport.ec.europa.eu/transport-themes/passenger-rights/national-enforcement-bodies-neb_en`, copiami nome ufficiale e sito di quei nove, e li aggiungo a `lib/lettera/neb.ts` in cinque minuti. |
